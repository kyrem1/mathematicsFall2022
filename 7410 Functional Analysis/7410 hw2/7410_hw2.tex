...
\begin{homeworkProblem}
  ...
  \begin{proof}
    Suppose that the statement fails. Then there is some $ \eps\in(0,1) $ at which it fails. Then for $ x\in X$ such that $ \norm{x} = 1 $, $ dist(x,Y)<1-\eps $. Let $ x_0 $ satisfy the previous statement and be such that $ x_0\not\in Y $. By Hahn Banach, there exists a $ \phi\in X^{*} $ such that $ \norm{\phi}= \frac{1}{dist(x_{0},Y)}$, $ \phi(x_0)=1 $, and $ \phi\vert_Y=0 $. So, $ \norm{\phi}>\frac{1}{1-\eps} $. \\


    %TODO Figure out how to do this.
    Let $ Q:X\to X/Y $ be the corresponding quotient map. For all $ x\in X $ with 
  \end{proof}
