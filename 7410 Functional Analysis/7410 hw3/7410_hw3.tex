...
\begin{homeworkProblem}
    ...
    \begin{proof}
        Let $ \iota: X\hookrightarrow \F^{X^{*}} $ be the canonical evaluation map, and for brevity write $ \hat{x} = \iota(x) $. Note that each $ \hat{x} $ is weak-$ * $ continuous, so $ \iota(X)\sub C(X^{*},wk^{*}) $. Let $ K = (Ball(X^{*}),wk^{*}) $, so $ K $ is a compact space by Banach-Alaoglu. Define a map $ \phi:X\to C(K) $ by $ \phi(x) = \iota(x)\vert_{Ball(X^{*})} $.

        % TODO show isometry and surjectivity using Hahn-Banach.
...
\begin{homeworkProblem}
    ...
    \textbf{(b)}: Show that the map $ \Phi:Bil(X\times Y,\F)\to B(X,Y^{*}) $ given by $ [{\Phi}(B)(x)](y)=B(x,y) $ is a well-defined, isometric isomorphism. 

    \begin{proof}\ \\
        \underline{well-defined}: For fixed $ x\in X $, we have that
        \[
            \norm{B_{x}} = \sup_{\norm{y}\leq 1} |B(x,y)| \leq \norm{B} <+\infty
        \]
        so $ B_{x} \in Y^{*} $.\\

        \underline{isometric}: $ B\in Bil(X\times Y,\F) $, $ x\in X $. On one hand, from 
        \[
            \norm{\Phi(B)} = \sup_{\norm{x}\leq 1}\norm{B(x,\cdot)}_{Y^{*}} = \sup_{\norm{x}\leq 1} \sup_{\norm{y}\leq 1} |B(x,y)|
        \]
        it is clear that $ \norm{\Phi(B)} \leq \norm{B} $. Suppose that $ x\in X $, $ y\in Y $, so $ |B(x,y)| \leq \norm{B}\norm{x}\norm{y} $. Then
        \[
            |B(x,y)| = |B_{x}(y)|\leq \norm{B_{x}}\norm{y} \leq \norm{\Phi(B)}\norm{x}\norm{y},
        \] 
        whence by minimality $ \norm{B}\leq \norm{\Phi(B)} $. Thus $ \Phi $ is isometric and $ \Phi(B)\in B(X,Y^{*}) $.\\
        

        \underline{isomorphism}: It suffices to show surjectivity. Suppose that $ \Gamma\in B(X,Y^{*}) $. Define a map $ B:X\times Y\to \F $ by $ B(x,y) = \Gamma(x)(y) $. For $ x\in X $ and $ y\in Y $, we have that 
        \[
            |B(x,y)| \leq \norm{\Gamma(x)}\norm{y}  \leq \norm{\Gamma} \norm{x} \norm{y},
        \]
        whence $ B\in Bil(X\times Y, \F) $. Moreover, by construction $ \Phi(B) = \Gamma $.
    \end{proof}
...
\begin{homeworkProblem}
    ...
    \begin{proof}
        Fix $ x\in X $, $ \eps>0 $ and choose $ z\in D $ such that $ \norm{x-z}<\eps $. Set $ C = \sup_{n}\norm{T_{n}} $. Then, we compute that 
        \begin{align*}
            \norm{T_{n}x-T_{m}x} &\leq \norm{T_{n}x-T_{n}z} + \norm{T_{n}z - T_{m}x} < C\eps + \norm{T_{n}z - T_{m}x} \\
            & \leq C\eps + \norm{T_{n}z-T_{m}z} + \norm{T_{m}z - T_{m}x} < 2C\eps + \norm{T_{n}z - T_{m}z} \xrightarrow{n,m\to\infty}0,
        \end{align*}
        so by completeness, $ T_{n}x $ converges for all $ x\in X $.\\

        Now define $ Tx = \lim_{n\to\infty}T_{n}x $. This map is well-defined and linear, so it suffices to show that it is bounded. But 
        \[
            \norm{Tx} = \lim_{n\to\infty}\norm{T_{n}x} \leq \liminf_{n\to\infty}\norm{T_{n}}\norm{x} \leq C\norm{x},
        \]
        whence $ \norm{T} \leq C $ so $ T $ is a bounded operator. Lastly, suppose that $ x\in X $. Then
    \end{proof}
...
\begin{homeworkProblem}
    ...
    \begin{proof}
        Since $ Y $ is norm-closed inside $ Y^{**} $, $ S $ is norm-continuous, and $ D\sub S^{-1}(Y) $, it follows that $ X\sub S^{-1}(Y) $ i.e. $ S(X)\sub Y $. Now, suppose that $ x\in X $ and $ \phi\in Y^{***} $. Then there is some $ y\in Y $ and $ \psi\in Y^{*} $ such that $ S(x) = \widehat{y} $ and $ \phi\vert_{Y} = \psi $.
        
        By construction, $ S(x)(\psi) = \lim_{n\to\infty} \psi(T_{n}x) $, so we compute that
        \[
            \phi(\widehat{T_{n}x}) = \widehat{\psi}\widehat{T_{n}x} = \widehat{T_{n}x}(\psi) = \psi(T_{n}x) \to S(x)(\psi) = \phi(S(x)) = \phi(\widehat{y}).
        \]
        Hence, $ T_{n}\to S $ WOT.
